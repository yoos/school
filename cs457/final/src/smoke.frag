...
float uKd = 0.7;
float uKs = 0.8;
float uShininess = 800.;
vec4 uColor = vec4(0, 0.3, 0.6, 1);
float uAlpha = 1.0;
...
void main()
{
  ...
  while (dist > 0.0 && fogDensity < 1.0) {
    fogDensity += 0.05/float(uFogStep);
    //fogDensity += uFogNoise * snoise(vECposition/8.0)/float(uFogStep);
    sampleLoc += normEye/float(uFogStep);
    dist -= 1./float(uFogStep);
  }

  // Step off more obviously
  fogDensity *= atan(length(vEye)*4.0-10.0)/3.14159 + 0.5;
